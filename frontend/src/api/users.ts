...
export async function uploadAvatar(file: File) {
  ...
  const { data } = await apiClient.patch<ApiResponse<UpdatedUser>>(
    "/users/avatar",
    formData,
    {
...
export async function deleteAvatar() {
  const { data } =
    await apiClient.delete<ApiResponse<UpdatedUser>>("/users/avatar");
  return data.data;
}
...
export async function registerUser(payload: {
  ...
  const { data } = await apiClient.post<
    ApiResponse<{ user: UpdatedUser; password: string }>
...
export async function adminUpdateUser(
  userId: string,
  payload: { first_name: string; last_name: string; role?: "admin" | "staff" },
) {
  const { data } = await apiClient.patch<ApiResponse<UpdatedUser>>(
    `/users/${userId}`,
    payload,
...
export async function adminResetPassword(userId: string) {
  const { data } = await apiClient.post<
    ApiResponse<{ username: string; temporaryPassword: string }>
...
export async function adminDeleteAvatar(userId: string) {
  const { data } = await apiClient.delete<ApiResponse<UpdatedUser>>(
    `/users/${userId}/avatar`,
  );
  return data.data;
}
