...
export async function getProducts() {
  const { data } = await apiClient.get<ApiResponse<Product[]>>('/inventory/products');
  return data.data;
}
...
export async function createProduct(payload: {
  ...
  const { data } = await apiClient.post<ApiResponse<Product>>('/inventory/products', formData, {
...
export async function updateProduct(
  ...
  const { data } = await apiClient.put<ApiResponse<Product>>(`/inventory/products/${id}`, formData, {
    headers: { 'Content-Type': 'multipart/form-data' },
  });
  return data.data;
}
...
export async function deleteProduct(id: string) {
  const { data } = await apiClient.delete<ApiResponse<Product>>(`/inventory/products/${id}`);
  return data.data;
}
