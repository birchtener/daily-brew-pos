...
export class OrdersService {
    ...
    try {
      const orderResult = await prisma.$transaction(async (tx) => {
        ...
        const processingLineItems = input.items.map((cartItem) => {
          const prod = targetProducts.find(
            (product) => product.id === cartItem.product_id,
          );

          if (!prod) {
            throw createHttpError(
              `Validation Failure: Product [${cartItem.product_id}] not found.`,
              404,
            );
          }

          const lineSubTotal = new Prisma.Decimal(prod.price).times(
            cartItem.quantity,
          );
        ...
        if (input.discount_code) {
          const promo = await tx.discount.findUnique({
            where: { code: input.discount_code },
          });
          if (promo) discountPercentage = promo.percentage;
        }

        const totalDeductionMultiplier = new Prisma.Decimal(
        ...
        for (const item of processingLineItems) {
          ...
          if (!input.park) {
            await this.executeStockDeductionLoop(
              tx,
              createdLine.id,
              item.product_id,
              item.quantity,
      ...
      throw createHttpError(
        "Validation Failure: Order processing failed.",
        500,
      );
    }
  }

  static async checkoutParkedOrder(
    orderId: string,
    input: z.infer<typeof FinalizeParkedOrderSchema>,
    userId: string,
  ) {
    const ingredientsToScan = new Set<string>();
    ...
    try {
      const updatedOrder = await prisma.$transaction(async (tx) => {
        ...
        if (!activeOrder || activeOrder.order_status !== "parked") {
          throw createHttpError(
            "Access Failure: Target order reference unavailable or already finalized.",
            400,
          );
        }
        ...
        const calculatedTotal = subTotal.times(
          new Prisma.Decimal(1 - discountPercentage / 100),
        );
      ...
      throw createHttpError(
        "Validation Failure: Parked order settlement failed.",
        500,
      );
    }
  }
  ...
  private static async executeStockDeductionLoop(
    ...
    for (const step of productRecipe) {
      const totalNeededVolumeInRecipeUnit = new Prisma.Decimal(step.quantity)
        .times(orderQuantity)
        .toNumber();
      ...
      const batchAllocations = await FIFOService.calculateFIFODeduction(
        tx,
        step.ingredient_id,
        convertedNeededVolume,
      );
    ...
    for (const ingredientId of ingredientIds) {
      setImmediate(() => {
        void StockMonitorService.checkThreshold(ingredientId).catch((err) =>
          console.error(
            `BACKGROUND WORKER CRASH: Threshold verify failed for Ingredient [${ingredientId}]:`,
            err,
          ),
        );
      });
  ...
  static async voidOrder(orderId: string, userId: string, reason: string) {
    const ingredientsToScan = new Set<string>();
    ...
    try {
      const voidedOrder = await prisma.$transaction(async (tx) => {
        ...
        if (!targetOrder || targetOrder.order_status === "cancelled") {
          throw createHttpError(
            "Access Failure: Target order reference unavailable or already cancelled.",
            400,
          );
        }
          ...
          for (const item of targetOrder.items) {
            for (const deduction of item.stock_deductions) {
              const currentRemaining = new Prisma.Decimal(
                deduction.batch.quantity_remaining,
              );
              const updatedRemaining = currentRemaining.plus(
      ...
      void AuditService.log({
        message: `POS ORDER VOIDED: Order [${orderId}] has been voided. Reason: "${reason}".`,
        category: LogCategory.order,
        type: LogType.warn,
        userId,
      });
  ...
  static async deleteParkedOrder(orderId: string, userId: string) {
    try {
      ...
      if (!targetOrder) {
        throw createHttpError(
          "Access Failure: Target order reference unavailable.",
          400,
        );
      }
