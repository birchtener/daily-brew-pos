    ...
    static async createProduct(input: CreateProductInput, userId: string) {
        try {
            ...
            if (isKnownPrismaError(error, 'P2002')) {
                throw createHttpError('Validation Failure: Product already exists.', 409);
            }

            if (isKnownPrismaError(error, 'P2003')) {
                throw createHttpError('Validation Failure: Related record missing or invalid.', 409);
            }

            throw createHttpError('Validation Failure: Product creation failed.', 500);
        }
    }
