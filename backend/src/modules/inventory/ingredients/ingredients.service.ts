...
export class IngredientsService {
    ...
    try {
      ...
      if (isKnownPrismaError(error, "P2002")) {
        throw createHttpError(
          "Validation Failure: Ingredient already exists.",
          409,
        );
      }

      throw createHttpError(
        "Validation Failure: Unable to create ingredient.",
        500,
      );
    }
  }
  ...
  static async updateIngredient(
    ...
    try {
      ...
      if (existingIngredient && existingIngredient.img_path) {
        const oldImg = existingIngredient.img_path;
      ...
      throw createHttpError(
        "Validation Failure: Ingredient update failed.",
        500,
      );
    }
  }
  ...
  static async deleteIngredient(id: string, userId: string) {
    ...
    try {
      const deletedIngredient = await prisma.$transaction(async (tx) => {
        ...
        await deleteImage(ingredientWithRelations.img_path);
      ...
      throw createHttpError(
        "Validation Failure: Ingredient deletion failed.",
        500,
      );
    }
  }
